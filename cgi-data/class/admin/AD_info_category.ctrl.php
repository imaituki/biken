...
class AD_info_category {
	...
	// 主テーブル
	var $_CtrTable   = "mst_info_category";
	var $_CtrTablePk = "id_category";

	// コントロール機能（ログ用）
	var $_CtrLogName = "お知らせカテゴリー";
	...
	function check( &$arrVal, $mode ) {
		...
		if( (strcmp($mode, "update") == 0) ) {
			$objInputCheck->entryData( "お知らせカテゴリーID", "all", $arrVal["id_category"], array( "CHECK_EMPTY", "CHECK_NUM" ), null, null );
		}
	...
	function insert( $arrVal, $arrSql = null ) {
		...
		$arrSql["display_num"] = "( SELECT IFNULL(max_num + 1, 1) FROM ( SELECT MAX(display_num) AS max_num FROM " . $this->_CtrTable . " ) AS max_num ) ";
	...
	function update( $arrVal, $arrSql = null ) {
		...
		// 更新条件
		$where = $this->_CtrTablePk . " = " . $arrVal["id_category"];
	...
	function delete( $id ) {
		...
		// 更新
		$res = $this->_DBconn->update( $this->_CtrTable, array("delete_flg" => 1), $arrSql, $this->_CtrTablePk . " = " . $id );
	...
	function GetOption() {
		...
		// SQL配列
		$creation_kit = array(  "select" => "id_category, title, display_flg",
								"from"   => $this->_CtrTable,
								"where"  => "delete_flg = 0 ",
								"order"  => "display_num ASC"
							);
		...
		if( !empty($arr_option) ){
			foreach( $arr_option as $val ){
				...
				if( $val["display_flg"] != 1 ){
					$res[$val["id_category"]] = $val["title"]. "(非表示)";
				}
			}
		}
