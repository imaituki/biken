...
class AD_information {
	...
	function check( &$arrVal, $mode ) {
		...
		$objInputCheck->entryData( "カテゴリー", "id_category" , $arrVal["id_category"] , array( "CHECK_EMPTY", "CHECK_NUM" ), null, null );
	...
	function GetSearchList( $search, $option = null ) {
		...
		if( !empty( $search["search_category"] ) ){
			$creation_kit["where"] .= "AND id_category = " . $search["search_category"] . " ";
		}
